...
const getQuestionTRX = (idx = 0) => {
    if (quiz.questions[idx] == undefined) {
        const result = fetchWrapper.get(`${API_URL}/get_question?id=${idx+1}`)
        if (result) {
            quiz.questions[idx] = result
            currentQuestionIndex = result.id
        } else {
            throw new Error(`Error loading question ${idx}+1`)
        }
    }
...
const sendCurrentAnswerTRX = () => {
    const result = fetchWrapper.post(`${API_URL}/submit_answer`, {
        question_id: currentQuestionIndex,
        correct: checkCorrectAnswerTRX()
    })
    if (result.msg != undefined && result.msg == 'already submitted')
        alert("Hai già votato!")
    if (result.msg != undefined && result.msg == 'time over')
        alert("Il tempo è scaduto!")
}

const checkCorrectAnswerTRX = () => {
    const correct = quiz.questions[currentQuestionIndex].entered[0] == quiz.questions[currentQuestionIndex].correct
    /* if (correct)
        alert("Giusto!")
    else
        alert("Sbagliato!") */
...
const init = () => {
    ...
    loadQuestion(getQuestionTRX(), true)
    // totalQuestions = fetchWrapper.get(`${API_URL}/count`).count
    // updateProgessBarStatus()
    // score = fetchWrapper.get(`${API_URL}/my_score`).score
    // document.querySelector("#my_score").textContent = score
}
...
const loadQuestion = async (question, init) => {
    ...
    ShowHideContinueButton(getQuestionTRX(currentQuestionIndex))
}
...
const SaveWrittenAnswers = () => {
    getQuestionTRX(currentQuestionIndex).entered[0] = document.getElementById(`questionTextarea`).innerHTML
}
...
const loadPreviousEnteredText = () => {
    let entered = getQuestionTRX(currentQuestionIndex).entered
    if (entered.length > 0) {
...
const loadNewQuestion = async (adjustment) => {
    ...
    let type = getQuestionTRX(currentQuestionIndex).type
    if (type == 'long' || type == 'short') {
        SaveWrittenAnswers()
    }
    if (type == 'single') {
        sendCurrentAnswerTRX()
    }
    ...
    if (canLoadNewQuestion(adjustment)) {
        ...
        document.querySelector('.quiz-question-text-container').textContent = "Hai risposto alla domanda! Ricarica la pagina quando sarà sbloccata la prossima."
        let buttonContainer = document.getElementById(`quiz-continue-button-container`)
        buttonContainer.style.display = `none`
        await MoveQuestionContainerMiddle()
        return
        if (adjustment == `previous-question-load`) {
            loadQuestion(getQuestionTRX(currentQuestionIndex))
            // Displays next question. Does nothing if no questions to load.
        } else if (adjustment == `next-question-load`) {
            loadQuestion(getQuestionTRX(currentQuestionIndex))
        }
    }
}
...
const selectAnswer = (key, previous) => {
    ...
    let answerText = answer.querySelector(`.quiz-answer-text-item`).textContent
    if (answer) {
...
const storeAnswers = (add, key) => {
    ...
    if (add) {
        if (getQuestionTRX(currentQuestionIndex).type == `single`) {
            getQuestionTRX(currentQuestionIndex).entered.length = 0
        }
        getQuestionTRX(currentQuestionIndex).entered.push(key)
        // For removing user's answers from the local object
    } else {
        getQuestionTRX(currentQuestionIndex).entered = getQuestionTRX(currentQuestionIndex).entered.filter(item => item !== key)
    }
    // Ensures there are no duplicate answers in array
    getQuestionTRX(currentQuestionIndex).entered = uniq(getQuestionTRX(currentQuestionIndex).entered)
}
...
/* const updateProgessBarStatus = () => {
    ...
    let value = Math.floor((calculateQuizProgress(quiz.questions) / totalQuestions) * 100)
